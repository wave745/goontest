...
import { Home, TrendingUp, Star, Clock, Video, Image, Coins, MessageCircle, ChevronLeft, ChevronRight } from 'lucide-react';
        ...
        <div>
          ...
          <nav className="space-y-2">
            {categoryItems.map((item) => (
              <Link key={item.href} href={item.href}>
                <a className="flex items-center gap-3 rounded-lg px-3 py-2 text-sm text-muted-foreground hover:text-foreground hover:bg-accent/10 transition-colors" data-testid={`link-${item.label.toLowerCase().replace(' ', '-')}`}>
                  <item.icon className="h-4 w-4" />
                  {!isCollapsed && item.label}
                </a>
              </Link>
